        ...
        for row in range(self.grid_rows):
            for col in range(self.grid_cols):
                ...
                if cell_n > 0:  # Avoid division by zero
                    ...
                    cell_var_x=cell_sum_square_dx-(2*combined.mu[row][col][0]*cell_sum_dx)+(cell_n*(combined.mu[row][col][0]**2))
                    cell_var_y=cell_sum_square_dy-(2*combined.mu[row][col][1]*cell_sum_dy)+(cell_n*(combined.mu[row][col][1]**2))
